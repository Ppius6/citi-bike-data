...
class DataPipeline:
    ...
    def create_db_engine(self):
        ...
        return create_engine(
            conn_str, pool_size=5, max_overflow=10, pool_timeout=30, pool_recycle=1800
        )
    ...
    def get_latest_processed_date(self) -> Optional[datetime]:
        ...
        try:
            metadata = MetaData()
            trips_table = Table(
                self.config["table_name"], metadata, autoload_with=self.engine
            )

            with self.engine.connect() as conn:
                result = conn.execute(func.max(trips_table.c.started_at)).scalar()
        ...
        try:
            ...
            if not duplicate_rides.empty:
                ...
                if len(duplicate_rides) > 0:
                    sample_duplicates = (
                        duplicate_rides.groupby("ride_id").head(2).head(5)
                    )
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            if not processed_dfs:
                self.logger.info("No new data processed.")
                return True
            ...
            self.logger.info(
                f"Combined DataFrame has {len(combined_df)} records after removing duplicates."
            )
